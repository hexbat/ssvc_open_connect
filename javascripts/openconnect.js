// openconnect Homepage JavaScript Enhancement - Clean Version
...
function setupScrollAnimations() {
    ...
    const animateElements = document.querySelectorAll(
        '.openconnect-stats, .features-section, .performance-section, ' +
        '.feature-card, .openconnect-stat, .openconnect-buttons, ' +
...
function setupCounterAnimations() {
    const stats = document.querySelectorAll('.openconnect-stat-number');
...
function createFloatingElements() {
    const hero = document.querySelector('.openconnect-hero');
...
function setupInteractiveElements() {
    ...
    if (window.innerWidth > 768) {
        const cards = document.querySelectorAll('.feature-card, .openconnect-stat');
        cards.forEach(card => {
    ...
    const buttons = document.querySelectorAll('.openconnect-btn');
    buttons.forEach(button => {
        button.addEventListener('click', createRipple);
    });
    
    // Stats card click effect
    const statCards = document.querySelectorAll('.openconnect-stat');
...
function setupThemeColorChange() {
    ...
    function updateThemeColor() {
        ...
        const hue = 209 + (scrollPercent * 30); // From purple to pink
        document.documentElement.style.setProperty('--openconnect-primary', `hsl(${hue}, 28%, 39%)`);
        ticking = false;
    }
