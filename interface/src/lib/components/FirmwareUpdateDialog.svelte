<script lang="ts">
	...
	let displayMessage = $derived.by(() => {
		if (currentStatus === 'error') return currentError || 'Обновление не удалось.';
		if (currentStatus === 'finished') return 'Обновление успешно завершено.';
		if (currentStatus === 'progress') return 'Обновление...';
		if (currentStatus === 'preparing') return 'Подготовка...';
